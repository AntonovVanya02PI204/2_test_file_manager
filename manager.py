...
def directory_work():  # работа с директориями
    ...
    if choise == 1:
        ...
        name = str(input())
        if "pythonProject8" not in name:
            print("Нельзя работать вне папки pythonProject8 ")
        else:
            os.mkdir(name)
    if choise == 2:
...
def file_work():  # работа с файлами
    ...
    choise = str(input())
    if "pythonProject8" not in choise:
        print("Нельзя работать вне папки pythonProject8")
    else:
        print(
            "1 - Создать файл" + "\n" + "2 - Удалить файл" + "\n" + "3 - Читать файл" + "\n" + "4 - Запись в файл" + "\n" + "5 - Переименовывание" + "\n" + "6 - Копирование")
...
while True:  # бесконечный цикл обработки запросов
    ...
    if ans == 1:
        ...
        os.system(
            '"C:\WINDOWS\system32\cmd.exe"')  # запуск cmd для более глубокой работы, например смены основной директории
    elif ans == 3:
        file_work()